...
async function loadRoom() {
  ...
  // Always fetch fresh student data to get latest room_id assignment
  const { data: freshStudent, error: sErr } = await supabase
    .from("students")
    .select("*, rooms(id, block, room_number, capacity, occupancy_count, type)")
    .eq("id", STUDENT.id)
    .single();

  if (sErr || !freshStudent) {
    panel.innerHTML = "<p style='color:var(--gray-400)'>Could not load room details. Please try again.</p>";
    return;
  }

  // Check both room_id (UUID FK) and room (text) columns
  const roomId = freshStudent.room_id;
  const roomText = freshStudent.room;
  ...
  // If we have room_id use it, otherwise look up by text room number
  let room = freshStudent.rooms || null;
  let occupants = [];

  if (!room && roomText) {
    // Try to find room by text room number
    const { data: foundRoom } = await supabase
      .from("rooms")
      .select("*")
      .eq("room_number", roomText)
      .limit(1)
      .single();
    room = foundRoom;
  }
  ...
  if (!room) {
    panel.innerHTML = `
      <div class="empty-state">
        <div class="empty-icon">🏠</div>
        <h3>Room: ${roomText || "Assigned"}</h3>
        <p>Your room is <strong>${roomText}</strong>. Full room details will appear once the room is fully set up in the system.</p>
      </div>`;
    return;
  }
  ...
  const roommates = occupants.filter(o => o.id !== freshStudent.id);
