...
: students.map(s => `<tr> <td>${s.full_name}</td> <td>${s.reg_number}</td> <td>${s.program}</td> <td>${s.level}</td> <td>${s.sex}</td> <td>${"-"}</td> <td>${s.rooms ?`${s.rooms.block}-${s.rooms.room_number}`: "—"}</td> <td> <button class="btn-sm btn-primary" onclick="openAssignModal('${s.id}','${escHtml(s.full_name)}')">Assign</button> ${s.room ?`<button class="btn-sm btn-danger" onclick="removeRoom('${s.id}')">Remove</button>` : ""} </td> </tr>`).join(””);
}
...
const filtered = allStudents.filter(s =>
  ...
  (!sex    || s.sex                 === sex)   &&
  (!status || "-"  === status)&&
  (!search || s.full_name.toLowerCase().includes(search) || s.reg_number.toLowerCase().includes(search))
);
...
if (!info) return;
info.innerHTML = `<h3>Block ${r.block} – Room ${r.room_number}</h3> <p><strong>Capacity:</strong> ${r.capacity} &nbsp; <strong>Occupied:</strong> ${r.occupancy_count}</p> <p><strong>Status:</strong> <span class="badge badge-${r.type}">${r.type}</span></p> <h4>Occupants</h4> ${occupants.length === 0 ? "<p style='color:#9ca3af'>No occupants</p>" :`<ul>${occupants.map(o=>`<li>${o.full_name} (${o.reg_number}) – ${o.program}</li>`).join(””)}</ul>`} `;
...
tbody.innerHTML = (students || []).map(s => ` <tr> <td>${s.full_name}</td> <td>${s.reg_number}</td> <td>${s.program}</td> <td>${s.level}</td> <td>${s.rooms?.block}-${s.rooms?.room_number}</td> <td> <button class="btn-sm btn-warning" onclick="openAssignModal('${s.id}','${escHtml(s.full_name)}')">Change</button> <button class="btn-sm btn-danger"  onclick="removeRoom('${s.id}')">Remove</button> </td> </tr>`).join(””) || `<tr><td colspan="6" style="text-align:center">No allocations found</td></tr>`;
}
...
tbody.innerHTML = (data || []).map(a => `<tr> <td>${a.students?.full_name || "—"}</td> <td>${a.students?.reg_number || "—"}</td> <td>${a.students?.program || "—"}</td> <td>${new Date(a.created_at).toLocaleDateString()}</td> <td><span class="badge badge-${a.status}">${a.status}</span></td> <td> ${a.status === "pending" ?`
...
` : "—"} </td> </tr>`).join(””) || `<tr><td colspan="6" style="text-align:center">No applications</td></tr>`;
}
...
return `<tr> <td>Block ${b}</td> <td>${bRooms.length}</td> <td>${cap}</td> <td>${occ}</td> <td>${cap - occ}</td> <td> <div class="util-bar-wrap"> <div class="util-bar" style="width:${pct}%"></div> </div> ${pct}% </td> </tr>`;
}).join(””);
}
}
...
if (tbody) {
tbody.innerHTML = (users || []).map(u => `<tr> <td>${u.username}</td> <td><span class="badge badge-${u.role}">${u.role}</span></td> <td> ${u.id !== ADMIN.id ?`<button class="btn-sm btn-danger" onclick="deleteUser('${u.id}','${escHtml(u.username)}')">Delete</button>` : "—"} </td> </tr>`).join(””);
}
}
...
if (!tbody) return;
tbody.innerHTML = (data || []).map(log => ` <tr> <td>${new Date(log.timestamp).toLocaleString()}</td> <td>${log.user}</td> <td>${log.action}</td> </tr>`).join(””) || `<tr><td colspan="3" style="text-align:center">No logs</td></tr>`;
}
...
return `<div class="notice-card priority-${n.priority} ${expired ? 'notice-expired' : ''}"> <div class="notice-header"> <span class="notice-title">${n.pinned ? "📌 " : ""}${n.title}</span> <span class="notice-priority">${n.priority.toUpperCase()}</span> </div> <p class="notice-msg">${n.message}</p> <div class="notice-meta"> By ${n.author} · ${new Date(n.created_at).toLocaleDateString()} ${n.expiry_date ?`· Expires ${new Date(n.expiry_date).toLocaleDateString()}` : ""} ${expired ? "<span class='expired-tag'>EXPIRED</span>" : ""} </div> <div class="notice-actions"> <button class="btn-sm btn-danger" onclick="deleteNotice('${n.id}')">Delete</button> <button class="btn-sm btn-secondary" onclick="togglePin('${n.id}',${!n.pinned})">${n.pinned?"Unpin":"Pin"}</button> </div> </div>`;
