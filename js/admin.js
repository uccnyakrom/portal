  ...
  tbody.innerHTML = students.length === 0
    ...
    : students.map(s => `
      <tr>
        ...
        <td>
          <button class="btn-sm btn-primary" onclick="openAssignModal('${s.id}','${escHtml(s.full_name)}')">Assign</button>
          ${s.room ? `<button class="btn-sm btn-danger" onclick="removeRoom('${s.id}')">Remove</button>` : ""}
        </td>
      </tr>`).join("");
}
